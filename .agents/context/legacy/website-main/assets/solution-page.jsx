// ============================================================
// Solution leaf-page TEMPLATE. One component renders every
// individual solution page from window.SOLUTION_DATA[slug].
// Edit here once → every page updates.
//   <SolutionPage slug="competition-discovery" />
// ============================================================

const SolutionPage = ({ slug }) => {
  const d = window.SOLUTION_DATA[slug];
  const S = window.SOLUTION_SHARED;
  const cat = window.SOLUTION_CATS[d.cat];

  React.useEffect(() => {window.lucide && window.lucide.createIcons();});

  // Sibling pages in the same category → "Built For" rail.
  const siblings = cat.children.filter((s) => s !== slug).map((s) => window.SOLUTION_DATA[s]);
  const leafHref = (s) => `solutions/${s}.html`;
      ...
      <section className="uc-hero reveal">
        <div className="container">
          ...
          <div className="hero-cta-row" style={{ marginTop: 8 }}>
            ...
            <a href={cat.href} className="btn ghost lg">All {cat.label.toLowerCase()} solutions</a>
          </div>
        </div>
      </section>
      ...
      <section className="uc-eco section reveal">
        <div className="container">
          ...
          <div className="uc-eco-grid">
            ...
            <div>
              <h3 className="uc-eco-col-label">Related Solutions</h3>
              <div className="uc-eco-links">
                {siblings.map((sib) =>
