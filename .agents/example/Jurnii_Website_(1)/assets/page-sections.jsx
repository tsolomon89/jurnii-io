    ...
    <section className="section reveal" id="resources">
      <div className="container">
        <div className="section-head">
          ...
          <h2 className="h2-section">{featuredOnly ? 'Field notes from the intelligence frontline.' : 'Every report, guide, and comparison.'}</h2>
      ...
      <div className="pricing-grid">
        {PLANS.map(p => (
          <div key={p.name} className={`plan ${p.featured ? 'featured' : ''}`}>
            ...
            <a href="contact.html" className={`btn ${p.ctaVariant} lg`} style={{ justifyContent: 'center' }}>{p.cta}</a>
          </div>
        ))}
      </div>
...
const FAQS = [
  ...
  { q: 'What data do you need from us for Jurnii MMM?', a: 'A clean 24-month history of spend by channel and campaign, plus KPI data (FTDs, deposits, NGR) at a compatible granularity — typically daily or weekly. We run a data-readiness assessment in the scoping call to identify any gaps before contracts are signed.' },
...
const FAQSection = () => {
  ...
  rE(() => { window.lucide && window.lucide.createIcons(); }, [open]);
  return (
    <section className="section reveal" id="faq">
      <div className="container">
        <div className="section-head centered">
          <p className="eyebrow"><span className="dot"/>FAQ</p>
          <h2 className="h2-section">Questions operators ask in the first call.</h2>
        </div>
...
const TEAM = [
  { name: 'Fraser Davidson', role: 'CEO & Co-founder', bio: '20 years in iGaming commercial strategy. Previously CCO at a Tier 1 European operator.', initials: 'FD', acc: 1 },
  { name: 'Tristan Dexter', role: 'CTO & Co-founder', bio: 'Built data platforms at scale across two prior B2B SaaS exits. Engineering lead.', initials: 'TD', acc: 2 },
  { name: 'Mitch Varley', role: 'Head of Data & Analytics', bio: 'Former Head of Data Science, sportsbook major. Owns the modelling layer.', initials: 'MV', acc: 3 },
  { name: 'Jeff Pelc', role: 'Investor & Advisor', bio: 'Lead investor. Specialist in scaling B2B data businesses from seed to Series B.', initials: 'JP', acc: 4 },
];
window.TEAM = TEAM;
      ...
      <div className="about-grid">
        ...
        <div>
          <div className="team-grid">
            {TEAM.map(t => (
              <div key={t.name} className={`team-card acc-${t.acc}`}>
          ...
          <div className="advisors-strip">
            <h4>Advisors &amp; Investors</h4>
            <ul>
              <li><b>John Gleasure</b><br/><span>Chairman, Playtech · Founder, DAZN · Founder, Perform Group</span></li>
              <li><b>Jeff Pelc</b><br/><span>Lead investor · Data business scaling specialist</span></li>
            </ul>
          </div>
        </div>
      </div>
