...
> **PROTECTED — do NOT delete `Ext_Calendar_Booking_ID` (Events, field id 991103000000793032).**
> It reads as "0 code writes" in THIS repo only because it is written by a different system:
> it is the authoritative external correlation key for the database-first booking backend
> (`tsolomon89/jurnii-io`), which matches every booking Meeting to its journey by
> `Ext_Calendar_Booking_ID == journeyId`. Deleting it would break Meeting-to-journey
> reconciliation for every current and future booking. Removed from the Tier-2 deletion batch on
> this basis; it must stay on the Events layout and must never be deleted.
